...
<div class="box">
  ...
  <section class="my-4 px-4 sm:px-6">
    <div class="bg-white dark:bg-gray-900">
      <div class="border dark:border-gray-900 rounded-md" v-if="messages.length">
        <div
          class="dark:border-gray-900"
          v-for="(item, index) in messages"
          :key="index"
          :class="index === messages.length - 1 ? '' : 'border-b'"
        >
          <div
            class="flex flex-row py-4 px-4 sm:px-8 font-semibold text-gray-700 dark:text-gray-300 bg-neutral-100 dark:bg-gray-900"
            v-if="item.who === 'ME'"
          >
            <span class="pr-4 flex-grow-0 flex-shrink-0 w-10 sm:w-12">ME:</span>
            <span class="flex-grow-1 dark:text-gray-300">{{ item.message.trim() }}</span>
          ...
          <div
            ...
            v-if="item.who === 'AI'"
          >
            <span class="font-semibold pr-4 flex-grow-0 flex-shrink-0 w-10 sm:w-12">AI:</span>
            <div clas="flex-grow-1 whitespace-pre-wrap" style="white-space: pre-wrap" >
              <div>
                <span v-if="index !== messages.length - 1 && item.message.trim().length === 0">
                  AI 也不知道该说啥了，祝你平安吧~
                </span>
                <span
                  v-if="!loading && index === messages.length - 1 && item.message.trim().length === 0"
                >
                  AI 也不知道该说啥了，祝你平安吧~
                </span>
                <span
                ...
                <span v-if="item.values==='2'">
            
<span v-if="item.message.length===0">图片加载中，耐心等待...</span>
            <span v-if="item.message.length!==0">
              <img class="product-image"  :src="item.message.trim()" height="200" width="200">
            </span>
                </span>
                <span v-else>
                  {{ item.message.trim() }}
                 ...
                 <button class="px-0.5 " @click="copy(item.message.trim())" style="color:darkgray"> 复 制</button>
                <span v-if="loading && index === messages.length - 1" class="blink"></span>
                <Ad v-else />
              </div>
            </div>
          </div>
      ...
      <div v-else class="py-24">
        ...
        <div class="text-center text-gray-500 px-12">基于 OpenAI 的 ChatGPT 自然语言模型人工智能对话</div>
      </div>
    </div>
  </section>
</template>
